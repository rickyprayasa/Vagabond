...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({ 
  ...
  const handleDeleteTrip = (index: number) => {
    if (confirm(language === 'id' ? 'Hapus rencana ini?' : 'Delete this itinerary?')) {
      const newTrips = [...savedTrips];
      newTrips.splice(index, 1);
      setSavedTrips(newTrips);
      localStorage.setItem('vagabond_saved_itineraries', JSON.stringify(newTrips));
    }
  };

  const totalDays = savedTrips.reduce((acc, trip) => acc + (trip.totalDays || 0), 0);
                  ...
                  {savedTrips.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {savedTrips.map((trip, idx) => (
                        <div key={idx} className="bg-white border-2 border-neo-black p-4 hover:shadow-neo transition-all group relative">
                          ...
                          <div className="mt-4 pt-3 border-t-2 border-dashed border-zinc-200 flex justify-between items-center">
                             <div className="flex items-center gap-4 text-xs font-mono text-zinc-500">
                                <span className="flex items-center gap-1"><Wallet className="w-3 h-3" /> {trip.estimatedCost.total}</span>
                             </div>
